...
import ProgressBarTimer from "./ProgressBarTimerForElectionStateWiseWithVoiceShortVersion";
...
const ElectionDesign = ({ startedIndex = 0 }) => {
  ...
  let state;
  const getData = async () => {
    setItem((current) => dataCompleteWithElectionStats[index]);
  ...
  return (
    <div className="pt-10 bg-gray-900">
      ...
      <div className="flex w-full justify-center items-center pt-2 relative">
        <span className="absolute top-full bg-gradient-to-b from-yellow-200/[1] to-yellow-500/[1] bg-clip-text text-transparent text-xl font-semibold z-20 border-yellow-400">
          270
          <span className="text-sm block -mb-[0px]">To Win</span>
        </span>
      ...
      <div className="px-10 flex justify-center gap-16 pt-12 h-screen w-full z-[100]">
        <div>
        ...
        {/* {data?.map((item?) => ( */}
        <div className="p-2 pb-0 mt-44 rounded">
          ...
          >
            ...
            <p className="text-wrap text-4xl relative mr-20">
              {item?.votes}
              <i className="text-xs text-left capitalize text-wrap ml-1 absolute top-2 leading-[1.3]">
                Presidential votes
              </i>
            </p>
          </div>
...
const Card = ({ name, img, votShare, color }) => {
  return (
    <div className={`border-4 border-${color}-500 p-1 w-[300px]`}>
      ...
      <div className={`bg-cyan-50 text-${color}-700 text-[4rem] p-3 font-bold`}>
