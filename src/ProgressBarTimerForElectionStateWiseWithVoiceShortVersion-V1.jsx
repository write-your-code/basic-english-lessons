...
const ThirtySecCounter = ({
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      ...
      // setTimeLeft((prevTimeLeft) => prevTimeLeft + timeToAdd.toFixed(0));
    }, interval);
    return () => {
  ...
  return (
    <div className="mt-0 mb-1 relative">
          ...
          <span className="text-5xl absolute w-full h-full text-cyan-50 flex items-center justify-center text-nowrap -top-36 font-semibold ">
            {diff >= 0 ? (
              diff > 0 ? (
                ...
                <span className={`p-1 w-[200px] block bg-cyan-50  text-black`}>
                  {diff.toFixed(1)}
                </span>
              )
            ) : (
      ...
      <div
        className={`w-full h-full flex items-center justify-center z-10 text-cyan-50 ${
          mode === "new"
            ? "text-cyan-50 text-5xl"
            : "text-cyan-50/[0.85] text-5xl"
        } top-0 ml-[0px] font-bold transition-all`}
        style={
          {
            // right: "100%",
            // left: `${progress * 100}%`,
      ...
      >
        <span>
          ...
          {/* {progress * 30 >= time ? orignalformat : Math.floor(progress * 30)} */}
          {progress * 30 >= time
            ? orignalformat
      ...
      >
        {/* <img
            src={`${img}`}
            alt=""
            className="w-[45px] h-[4px] rounded-full object-cover"
          /> */}
      </span>
      {/* </div> */}
      {/* audios */}
